...
def create_query(user_query, is_synonym_used, click_prior_query, filters, sort="_score", sortDir="desc", size=10, source=None):
    name_field = "name.synonyms" if is_synonym_used else "name"
    query_obj = {
        'size': size,
        "sort": [
            {sort: {"order": sortDir}}
